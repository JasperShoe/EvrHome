...
public class CameraMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.position += MixedRealityCamera.transform.forward * speed * Time.deltaTime * 0.008f;
        count++;

        float length = 10;
        Vector3 rotation = Camera.main.transform.TransformDirection(Vector3.forward);
        Vector3 position = Camera.main.transform.position;
        Debug.DrawRay(position, rotation*length, Color.green);
    }
}